...
<tr><td align="right">AP Mode:</td><td><input type="checkbox" id="ap_mode" name="ap_mode"></td></tr>
...
void send_network_configuration_html()
{
	...
	if (server.args() > 0 )  // Save Settings
	{
		...
		for ( uint8_t i = 0; i < server.args(); i++ ) {
			...
			if (server.argName(i) == "ap_mode") config.isAP = true;
		}
...
void send_network_configuration_values_html()
{
	...
	values += "ap_mode|" +  (String) (config.isAP ? "checked" : "") + "|chk\n";
